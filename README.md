...
## 概要
チャットができるアプリケーションを開発しました。これはクライアントがサーバーリソースを使用して独自のチャットルームを開始およびホストできる
分散システムです。ユーザー管理には、各クライアントに対して一意のトークンを割り当て、安全な識別のためにユーザーをメモリ内のハッシュテーブル
で追跡しています。通信には、ソケットプログラミングを利用してプロセス間通信（IPC）を可能にしました。チャットルームの参加や作成、トークン生成
などの重要な操作では、SOCK_STREAM型ソケットを選択しました。これらのTCP接続は、順序が保証され、信頼性があり、双方向通信が可能であり、アクセ
...
アプリケーションのユニークな特徴の一つは、言語に依存しない設計です。異なるプログラミング言語間で通信できるクライアントサーバープログラムを作成し
、自身で設計したアプリケーションレイヤープロトコルを介して相互運用性を確保しました。このプロトコルは、データストリーム内のビットの解釈方法を詳細
...
グループチャットができるアプリケーションと言えば、日本ではLINEが有名ですね。
